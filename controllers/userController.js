...
// filter
const addFilder = async (req,res)=>{
    let date_ob = new Date();
 
    const PropertyType = req.body.PropertyType;
    const city = req.body.city;
    const state = req.body.state; 
    const PropertyBuyType= req.body.PropertyBuyType;
    try{
        if(PropertyType == "Residential"){
            //Residential
            res.json({message: "Get Successfully Data",status: true,data:[],date:date_ob})
        }
        else
        if(PropertyType == "Commercial"){
            // Commercial
            if(PropertyBuyType == "Sale"){
                const userimg = await   db.promise().query(`SELECT * FROM property_commercial_sale_cs WHERE cs_location_state = '${state}' AND cs_location_city = '${city}'`);
                
             
                var result = [];
                for (let i = 0; i < userimg[0].length; i++) {
                    const id = userimg[0][i].cs_property_id;

                    const userimgName = await   db.promise().query(`SELECT * FROM property_images WHERE property_id = '${id}'`);





                    if(userimgName[0].length >=1){
                        result.push({
                            property_img: userimgName[0][0].images_path,
                            property_id: userimg[0][i].cr_property_id,
                            detail_description: userimg[0][i].cr_detail_description,
                            detail_property_type: userimg[0][i].cr_detail_property_type,
                            detail_building_type: userimg[0][i].cr_detail_building_type,
                            detail_prop_age: userimg[0][i].cr_detail_prop_age,
                            detail_floor: userimg[0][i].cr_detail_floor,
                            detail_total_floor: userimg[0][i].cr_detail_total_floor,
                        
                            detail_builtup_area: userimg[0][i].cr_detail_builtup_area,
                            detail_furnishing: userimg[0][i].cr_detail_furnishing,
                            location_state: userimg[0][i].cr_location_state,
                            location_city: userimg[0][i].cr_location_city,
                            location_latitiude: userimg[0][i].cr_location_latitiude,
                        
                        
                            location_longitude: userimg[0][i].cr_location_longitude,
                            location_iframe: userimg[0][i].cr_location_iframe,
                            rental_detail_rent: userimg[0][i].cr_rental_detail_rent,
                            rental_detail_exp_deposit: userimg[0][i].cr_rental_detail_exp_deposit,
                            rental_detail_is_nogotiable: userimg[0][i].cr_rental_detail_is_nogotiable,
                        
                        
                            rental_detail_monthly_maintenance: userimg[0][i].cr_rental_detail_monthly_maintenance,
                            rental_detail_avail_from: userimg[0][i].cr_rental_detail_avail_from,
                            rental_detail_lease_duration: userimg[0][i].cr_rental_detail_lease_duration,
                            rental_detail_lockin_period: userimg[0][i].cr_rental_detail_lockin_period,
                            amenities_power_backup: userimg[0][i].cr_amenities_power_backup,
                        
                            amenities_lift: userimg[0][i].cr_amenities_lift,
                            amenities_parking: userimg[0][i].cr_amenities_parking,
                            amenities_washroom: userimg[0][i].cr_amenities_washroom,
                            amenities_water_storage: userimg[0][i].cr_amenities_water_storage,
                            amenities_security: userimg[0][i].cr_amenities_security,});
                    }
                    else
                    {
                        result.push({
                ...
                res.json({message: "Get Successfully Data",status: true,data:result,date:date_ob})
            }
            else
            if(PropertyBuyType == "Rent"){
                // Rent
                const userimg = await   db.promise().query(`SELECT * FROM property_commercial_rent_cr WHERE cr_location_state = '${state}' AND cr_location_city = '${city}'`);
                
         
                var result = [];
                for (let i = 0; i < userimg[0].length; i++) {

                    const id = userimg[0][i].cr_property_id;
        ...
        if(PropertyType == "Plot"){
            // Plot
            const userimg = await   db.promise().query(`SELECT * FROM property_plot_sale_ps WHERE ps_location_state = '${state}' AND ps_location_city = '${city}'`);
            
            var result = [];
            for (let i = 0; i < userimg[0].length; i++) {
                const id = userimg[0][i].ps_property_id;
                ...
                if(userimgName[0].length >=1){
                    result.push({
                        property_img: userimgName[0][0].images_path,
                        property_id: userimg[0][i].ps_property_id,
                        detail_description: userimg[0][i].ps_detail_description,
                        detail_plot_length: userimg[0][i].ps_detail_plot_length,
                        detail_plot_width: userimg[0][i].ps_detail_plot_width,
                        detail_width_of_facing_road: userimg[0][i].ps_detail_width_of_facing_road,
                        detail_has_boundary: userimg[0][i].ps_detail_has_boundary,
                        detail_inside_gated_project: userimg[0][i].ps_detail_inside_gated_project,
                    
                        rules_allowed_floors: userimg[0][i].ps_rules_allowed_floors,
                        location_state: userimg[0][i].ps_location_state,
                        location_city: userimg[0][i].ps_location_city,
                        location_latitiude: userimg[0][i].ps_location_latitiude,
                        location_longitude: userimg[0][i].ps_location_longitude,
                    
                    
                        sale_detail_price: userimg[0][i].ps_sale_detail_price,
                        sale_detail_available_from: userimg[0][i].ps_sale_detail_available_from,
                        sale_detail_is_negotiable: userimg[0][i].ps_sale_detail_is_negotiable,
                        sale_detail_currently_under_loan: userimg[0][i].ps_sale_detail_currently_under_loan,
                        amenities_water: userimg[0][i].ps_amenities_water,
                    
                    
                        amenities_sewage_connection: userimg[0][i].ps_amenities_sewage_connection,
                        amenities_gated_security: userimg[0][i].ps_amenities_gated_security,
                        amenities_electricity_connection: userimg[0][i].ps_amenities_electricity_connection,
                        info_ownership: userimg[0][i].ps_info_ownership,
                        info_khata_certificate_is_available: userimg[0][i].ps_info_khata_certificate_is_available,
                    
                        ps_info_conversion_certificate_is_available: userimg[0][i].ps_info_conversion_certificate_is_available,
                        ps_info_sale_deed_certificate_is_available: userimg[0][i].ps_info_sale_deed_certificate_is_available,
                        ps_info_encumbrance_certificate_is_available: userimg[0][i].ps_info_encumbrance_certificate_is_available,
                        ps_info_is_rera_approved: userimg[0][i].ps_info_is_rera_approved});
                }
                else
                {
                    result.push({
            ...
            res.json({message: "Get Successfully Data",status: true,data:userimg[0],date:date_ob})
        }
    }catch(error){
        res.json({message: "Something was wrong",status: false,data:error,date:date_ob})
    }
...
const add_property =async (req,res)=>{
    ...
    try{
        ...
        const userimg = await   db.promise().query(`INSERT INTO property_images (property_id, images_squence, images_path, images_alt, images_type, images_description) VALUES ('${proId}', '', '${PropertyImage}', '', '', '')`);
...
module.exports= {user_all, signle_user, add_user, update_user, delete_user,singleUpload, login,UserType, 
    ...
    add_service,service_all, delete_service, signle_services, update_service, addFilder}